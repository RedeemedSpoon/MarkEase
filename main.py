...
class MarkEase:
    ...
    def import_image(self):
        ...
        if self.file_path:
            try: 
                ...
                self.image = self.image.resize((self.new_width, self.new_height), Image.ADAPTIVE)
                self.photo = ImageTk.PhotoImage(self.image)
    ...
    def draw(self, x, y):
        ...
        self.real_mouse_x = int((x - ((1280 - self.new_width) / 2) - 60) * self.width_diff)
        self.real_mouse_y = int((y - ((720 - self.new_height) / 2) + 5) * self.height_diff)
    ...
    def draw_text(self, event):
        ...
        self.real_font = ImageFont.load_default(int(float(self.font_size) * self.width_diff))
        self.drawing.text((self.mouse_x, self.mouse_y), self.user_text, fill=self.rgba, font=self.font)
        self.real.text((self.real_mouse_x, self.real_mouse_y), self.user_text, fill=self.rgba, font=self.real_font)
        self.update_image()   
    ...
    def draw_shape(self, event):
        ...
        real_x2, real_y2 = real_x1 + (self.size * self.width_diff), real_y1 + (self.size * self.height_diff)
    ...
    def shape(self, finish=False):
        if finish:
            ...
            ttk.Label(self.shape_window, text="Select Shape:").grid(row=0, column=0, padx=10, pady=20)
            self.shape_var = tk.StringVar(value="Rectangle")
            ttk.Radiobutton(self.shape_window, text="Rectangle", variable=self.shape_var, value="Rectangle").grid(row=0, column=1, padx=10, pady=10)
            ttk.Radiobutton(self.shape_window, text="Circle", variable=self.shape_var, value="Circle").grid(row=0, column=2, padx=10, pady=10)

            ttk.Label(self.shape_window, text="How big is the shape?").grid(row=1, column=0, padx=10, pady=10)
            self.shape_resolution = ttk.Combobox(self.shape_window, values=self.thing_width, width=10)
            self.shape_resolution.set("Small")
            self.shape_resolution.grid(row=1, column=1, columnspan=2, padx=10, pady=10)

            ttk.Button(self.shape_window, text="OK", command=lambda: self.shape(True)).grid(row=3, column=0, padx=30, pady=30)
            ttk.Button(self.shape_window, text="Cancel", command=self.shape_window.destroy).grid(row=3, column=2, padx=30, pady=30)
    ...
    def print_watermark(self, event):
        ...
        real_watermark_resized = self.watermark_img.resize((int(new_width * self.width_diff), int(new_height * self.height_diff)))
        mask = watermark_resized.convert("L").point(lambda x: int(x * (float(self.opacity) / 255)))
        real_mask =real_watermark_resized.convert("L").point(lambda x: int(x * (float(self.opacity) / 255)))
        self.image.paste(watermark_resized, (self.mouse_x, self.mouse_y), mask=mask)
        self.og.paste(real_watermark_resized, (self.real_mouse_x, self.real_mouse_y), mask=real_mask)
        self.update_image()
        self.canvas.unbind("<Button-1>")
        ...
        if finish:
            ...
            if self.file_path:
                try: 
                    ...
                    self.image_resolution = ttk.Combobox(self.watermark_window, values=self.thing_width, width=10)
    ...
    def opacity_layer(self, finish=False):
        if finish:
            ...
            self.slider = tk.Scale(self.opacity_window, from_=0, to=255, orient="horizontal", length=200)
